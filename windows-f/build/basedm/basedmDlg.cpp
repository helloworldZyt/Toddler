...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LOGVIEW, CStatic_Log_View);
...
void CbasedmDlg::myDbgOut(const char* s)
{
	...
	if (6<maxline++)
	{
		CStatic_Log_View.SetWindowText(_T(""));
		maxline=0;
	}
	
	CStatic_Log_View.GetWindowText(csOld);
	
	csNew.Format(_T("%s\n\r"), s);
	csOld += csNew;
	CStatic_Log_View.SetWindowText(csOld);
	return;
}
...
void CbasedmDlg::OnBnClickedStart()
{
	...
	dbgOut("you click start %d times!", cntstart++);
}
...
void CbasedmDlg::OnBnClickedDoit()
{
	// TODO: 在此添加控件通知处理程序代码
	static int cntdo = 0;
	dbgOut("you click start %d times!", cntdo++);
}
